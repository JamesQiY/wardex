...
const Card = ({ playerArr, darkTheme }) => {
  ...
  return (
    <div className="player_cards overflow-auto" id={player.name}>
      <div className="player_title"> {player.name} </div>
      <div className="info-container flex flex-row flex-grow">
        ...
        <div className="m-2 p-2 flex-grow"><Graph playerData={player} darkTheme={darkTheme} key={darkTheme} /></div>
      </div>
    </div>
  )
}
